...
describe('LocalFilesService', () => {
    ...
    beforeEach(async () => {
        ...
        localFilesRepository = module.get<Repository<LocalFile>>(getRepositoryToken(LocalFile));
    });
    ...
    describe('saveLocalFileData', () => {
        it('should create a local file', async () => {
            const localFileDto = { filename: 'filename', path: 'some/path', mimetype: 'mimetype' }
    ...
    describe('findById', () => {
        it('should return a local file when found', async () => {
            jest.spyOn(localFilesRepository, 'findOne').mockResolvedValue(mockLocalFile);
