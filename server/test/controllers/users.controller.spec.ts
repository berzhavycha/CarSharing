...
import { UpdateUserDto, UpdateUserBalanceDto } from '@/dtos';
...
describe('UsersController', () => {
    ...
    describe('updateUser', () => {
        it('should update user details', async () => {
            ...
            const result = await usersController.updateUser(mockUserId, mockUpdateUserDto);

            expect(result).toBe(mockUpdatedUser);
            expect(usersService.updateUser).toHaveBeenCalledWith(mockUserId, mockUpdateUserDto);
        });
    });
    ...
    describe('topUpUserAccount', () => {
        it('should top up user account balance', async () => {
            ...
            jest.spyOn(usersService, 'updateUserBalance').mockResolvedValue(mockUpdatedUser);

            const result = await usersController.topUpUserAccount(mockUserId, mockUpdateUserBalanceDto);
